# **Cybersecurity & Systems Engineering Portfolio**

<div align="center">

## **Building secure, data‑driven systems that elevate operational resilience**

</div>

---
### **Paul Leone** | *Aspiring Security Analyst & Infrastructure Engineer*



"Welcome to My Professional Portfolio"
    This site showcases my work across security engineering, data analytics, threat intelligence, and hands‑on homelab design. Each project includes architecture, methodology, and practical outcomes.

---

# Projects

| Project | Description |
|--------|-------------|
| :material-chart-line: **Business Intelligence & Data Analytics** | End‑to‑end analytics workflow demonstrating data modeling, dashboarding, and operational insights. |
| :material-shield-search: **Threat Intelligence Brief & SOC Improvement Plan** | Threat‑informed analysis with MITRE ATT&CK mapping, detection opportunities, and SOC maturity recommendations. |
| :material-home-analytics: **Security Homelab** | Hands‑on security engineering environment featuring SIEM, detection pipelines, and network segmentation. |

---

## Example Project Card

=== "Featured Project"

    ???+ abstract "Security Homelab — Hands‑On Engineering Environment"
        **A fully segmented, detection‑focused homelab** built to simulate real SOC workflows.  
        Includes SIEM ingestion pipelines, network segmentation, detection engineering, and adversary emulation.

        [:material-arrow-right-circle: View Project](projects/homelab.md)

---

# What I Do

- :material-shield-cog: **Security Engineering** — building detection pipelines, log ingestion workflows, and monitoring architectures.  
- :material-target: **Threat‑Informed Defense** — mapping adversary behaviors to detections, controls, and SOC processes.  
- :material-database-search: **Data Analytics & Automation** — transforming raw data into dashboards, insights, and automated workflows.  
- :material-cloud-outline: **Cloud & Infrastructure** — designing secure, scalable environments for experimentation and learning.  
- :material-file-document-edit: **Technical Writing & Documentation** — creating clear, structured documentation for complex systems.

---

# Career Focus

I’m focused on roles where I can combine **security engineering**, **analytics**, and **operational problem‑solving**. I’m especially drawn to environments that value curiosity, continuous improvement, and building systems that make teams more effective.

---

# Resume

- :material-file-account: **Resume (PDF)**

---

# Connect

- :material-linkedin: **LinkedIn**  
- :material-github: **GitHub**  
- :material-email: **Email**
---
:material-chart-line:

:material-shield-search:

:material-home-analytics:

:material-target:

:material-file-account:

:material-linkedin:

:material-github:

:material-email: